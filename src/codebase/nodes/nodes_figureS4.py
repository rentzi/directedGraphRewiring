...
def run_analysis(A, REP, P_RAND):
    ...
    for rep in np.arange(REP):
        for indP, p in enumerate(P_RAND):
            ...
            logging.info(
                "ave inverse path length is "
                + str(average_inv_path_len)
                + " and number of possible paths "
                + str(num_of_possible_paths)
            )
            logging.info("average path is " + str(path_lens[rep, indP]))
    return path_lens, possible_pathsAll


def plot_panel_2(P_RAND, possible_pathsAll):
    path_std = np.std(possible_pathsAll, axis=0)
    path_mean = np.mean(possible_pathsAll, axis=0)
    plt.errorbar(P_RAND, path_mean, path_std, marker="o", color="g", linewidth=2)
    plt.xlabel("Padvection")
    plt.ylabel("Number of pairs with a path")
    plt.show()


def plot_panel_1(P_RAND, path_lens):
    path_lens_std = np.std(path_lens, axis=0)
    path_lens_mean = np.mean(path_lens, axis=0)
    plt.errorbar(
        P_RAND, path_lens_mean, path_lens_std, marker="d", color="r", linewidth=2
    )
    plt.xlabel("Padvection")
