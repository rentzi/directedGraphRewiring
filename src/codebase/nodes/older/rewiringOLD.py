...
def get_nodes_receiving_and_sending_edges(adj_matx, n_nodes):
    """
...
def pick_node_with_in_out_edges(
    adj_matx, n_nodes, tau, p_rnd_rewire,
):
    """
...
def compute_consensus_kernel(adj_matx, tau):
    ...
    Din = np.diag(np.sum(adj_matx, axis=0))
    Lin = Din - adj_matx
...
def compute_advection_kernel(adj_matx, tau):
    ...
    Dout = np.diag(np.sum(adj_matx, axis=1))
    Lout = Dout - adj_matx
...
def run_out_dynamics(
    AInit, p_rnd_rewire, n_rewirings, tau, flag_alg = 'consensus',
):
    ...
    is cut, and a connection is added with the head being k. It operates in the columns Works
    ...
    for iter_i in range(n_rewirings):
        ...
        # randomly pick a node "x" with incoming & outgoing edges
        (not_x, node_x,) = pick_node_with_in_out_edges(
            adj_matx, n_nodes, tau, p_rnd_rewire,
        )

        # find the nodes with no incomings from "x"
        x_non_tail_bool = 1.0 * np.logical_not(adj_matx[not_x, node_x,])
        ...
        if np.random.random_sample() >= p_rnd_rewire:
            ...
            x_rewired_tail = x_tails[np.argmin(kernel[x_tails, node_x,])]

            # Get x's hottest non-tail (excluding "x")
            x_non_tails = not_x[x_non_tail_bool.nonzero()[0]]
            x_rewired_non_tail = x_non_tails[np.argmax(kernel[x_non_tails, node_x,])]

        else:  # else we randomly rewire

            # randomly pick one of x's tail (excluding "x")
            x_tails = adj_matx[not_x, node_x,].nonzero()[0]
            x_rewired_tail = np.random.choice(x_tails)

            # randomly pick one of x's non tails (excluding "x")
            x_rewired_non_tail = not_x[np.random.choice(x_non_tail_bool.nonzero()[0])]

        # Warning if exception
        if x_rewired_tail == x_rewired_non_tail:
            print("Problem")
            print(
                "The A nodes rewired are %d and %d with weight %f"
                % (x_rewired_non_tail, node_x, adj_matx[x_rewired_tail, node_x,],)
            )
            print("The A nodes disconnected are %d and %d" % (x_rewired_tail, node_x,))
        ...
        adj_matx[x_rewired_non_tail, node_x,] = adj_matx[
            x_rewired_tail, node_x,
        ]
        adj_matx[x_rewired_tail, node_x,] = 0
...
def run_in_dynamics(
    ...
    """
    Rewires iteratively a matrix A. At each iteration the rewiring can be random
    (probability = p_rnd_rewire) or according to a consensus function (probability = 1-p_rnd_rewire).
    ...
    for iter_i in range(n_rewirings):
        ...
        x_non_head_bool = 1.0 * np.logical_not(adj_matx[node_x, not_x,])
        ...
        if np.random.random_sample() >= p_rnd_rewire:  # rewire by network diffusion
            ...
            x_rewired_head = x_heads[np.argmin(kernel[node_x, x_heads,])]

            # get node x's hottest non-head (excluding "x")
            x_non_heads = not_x[x_non_head_bool.nonzero()[0]]
            x_rewired_non_head = x_non_heads[np.argmax(kernel[node_x, x_non_heads,])]

        else:  # now we just randomly rewire

            # randomly pick one of x's heads (excluding "x")
            x_rewired_head = np.random.choice(adj_matx[node_x, :,].nonzero()[0])

            # randomly pick one of x's non-heads (excluding "x")
            x_rewired_non_head = not_x[np.random.choice(x_non_head_bool.nonzero()[0])]

        if x_rewired_head == x_rewired_non_head:

            print("Problem")
            print(
                "The A nodes rewired are %d and %d with weight %f"
                % (x_rewired_non_head, node_x, adj_matx[x_rewired_head, node_x,],)
            )
            print("The A nodes disconnected are %d and %d" % (x_rewired_head, node_x,))

        # cut and rewire in-degree
        adj_matx[node_x, x_rewired_non_head] = adj_matx[node_x, x_rewired_head]
        adj_matx[node_x, x_rewired_head] = 0
...
def run_in_and_out_dynamics(
    ...
    for iter_i in range(n_rewirings):
        ...
        if (x_rewired_head == x_rewired_non_head) | (
            x_rewired_tail == x_rewired_non_tail
        ):
            print("Problem")
        ...
        adj_matx[x_rewired_non_tail, node_x] = adj_matx[x_rewired_tail, node_x]
        adj_matx[x_rewired_tail, node_x] = 0
...
def run_dynamics_advection_consensus_sequence(
    AInit, p_rnd_rewire, n_rewirings, tau,
):
    ...
    for iter_i in range(n_rewirings):
        ...
        if (iter_i%2) == 0:
            ...
            # ====== REWIRING FOR THE out-DEGREE OF THE NODE X ====
            kernel = compute_consensus_kernel(adj_matx, tau)
            ...
            # ====== REWIRING FOR THE in-DEGREE OF NODE X =====
...
##  run BOTH advection or consensus at each iteration
def run_dynamics_advection_consensus_parallel(
    ...
    for iter_i in range(n_rewirings):
        ...
        # ====== CONSENSUS FOR THE out-DEGREE OF THE NODE X ====
        kernel_cons = compute_consensus_kernel(adj_matx, tau)

        # get node x's coldest tail (excluding "x")
        x_tails = adj_matx[:, node_x].nonzero()[0]
        x_rewired_tail = x_tails[np.argmin(kernel_cons[x_tails, node_x,])]

        # Get x's hottest non-tail (excluding "x")
        x_non_tails = not_x[x_non_tail_bool.nonzero()[0]]
        x_rewired_non_tail = x_non_tails[np.argmax(kernel_cons[x_non_tails, node_x,])]

        # ====== ADVECTION FOR THE in-DEGREE OF NODE X =====
        kernel_adv = compute_advection_kernel(adj_matx, tau)

        # get node x's coldest head (excluding "x")
        x_heads = adj_matx[node_x, :,].nonzero()[0]
        x_rewired_head = x_heads[np.argmin(kernel_adv[node_x, x_heads,])]

        # get node x's hottest non-head (excluding "x")
        x_non_heads = not_x[x_non_head_bool.nonzero()[0]]
        x_rewired_non_head = x_non_heads[np.argmax(kernel_adv[node_x, x_non_heads,])]
